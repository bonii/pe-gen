...
IMPORT int _sanity(const char *file, unsigned int line);
...
static void DoPrefix(unsigned int line);
...
# define Delay(A) sleep((unsigned int) A)
#elif defined(AMIGA)
...
_db_enter_(const char *_func_,
    ...
    const char **_sfunc_,
    const char **_sfile_,
    unsigned int * _slevel_, char ***_sframep_ __attribute__ ((unused)))
{
...
void
_db_return_(unsigned int _line_,
    const char **_sfunc_, const char **_sfile_, unsigned int * _slevel_)
{
    CODE_STATE *state;
...
void
_db_pargs_(unsigned int _line_, const char *keyword)
{
...
void
_db_dump_(unsigned int _line_, const char *keyword, const char *memory, unsigned int length)
{
    int pos;
    char dbuff[90];
    CODE_STATE *state;
	...
	pos = 0;
	while (length-- > 0) {
	    unsigned int tmp = *((unsigned char *) memory++);
	    if ((pos += 3) >= 80) {
		fputc('\n', _db_fp_);
		pos = 3;
	    }
